 ...
 */
function convertToTensors(data: any, targets: any, testSplit: any) {
 ...
 *     {0, 1, 2}.
 */
export function getIrisData(testSplit: any) {
    return tf.tidy(() => {
        const dataByClass: any[] = [];
        const targetsByClass: any[] = [];
        for (let i = 0; i < IRIS_CLASSES.length; ++i) {
            dataByClass.push([]);
            targetsByClass.push([]);
        }
        for (const example of IRIS_DATA) {
            const target: any = example[example.length - 1];
            const data: any = example.slice(0, example.length - 1);
